...
export function BSTLab() {
  ...
  const [msg, setMsg] = useState<ReactNode>(
    "输入一个 0–99 的整数,插入或查找 —— 观察它怎么从根「一路二分下坠」。",
  );
  ...
  const doInsert = async () => {
    ...
    if (!found && count >= CAP) {
      setMsg(`实验室最多容纳 ${CAP} 个节点 —— 点「重置示例树」再玩。`);
      setBusy(false);
      return;
    }
    let acc: number[] = [];
    ...
    if (found) {
      ...
      setMsg(
        <>
          <b>{v}</b> 已经在树里 —— BST 不存重复值(工程里常在节点上挂个计数器)。
        </>,
      );
      setBusy(false);
      return;
    ...
    setMsg(
      path.length === 0 ? (
        ...
        <>
          走到 null,就地安家!比较了 <b>{path.length}</b> 次 =
          根到落点的路径长度 —— 插入永远发生在「叶子空位」,代价 <b>O(h)</b>。
        </>
      ),
    );
    setBusy(false);
  ...
  const doSearch = async () => {
    ...
    if (found) {
      ...
      setMsg(
        <>
          找到 <b>{v}</b>!一共比较 <b>{path.length}</b> 次。树里 {count} 个节点,
          却只看了一条路径上的 {path.length} 个 —— 这就是 <b>O(h)</b>。
        </>,
      );
    } else {
      ...
      setMsg(
        <>
          在 <b>{path[path.length - 1]}</b> 处该拐的方向是 null —— 撞到空位,
          说明<b>整棵树都不可能有 {v}</b>(有的话它必然住在这条路径上)。
        </>,
      );
    }
    setBusy(false);
  ...
  const doWorst = async () => {
    ...
    setInput("");
    setRoot(null);
    setMsg("清空。现在按 1 → 2 → 3 → 4 → 5 的顺序插入(有序数据)…");
    await sleep(700);
    let r: BN | null = null;
    let acc: number[] = [];
    for (let v = 1; v <= 5; v++) {
    ...
    setMsg(
      <>
        <b>5 个节点,高度 5</b> —— 树「斜」成了一条链,查 5 要比较 5 次:
        O(h) 变成了 O(n),BST 退化成链表。有序输入是 BST 的天敌,
        这正是 §05 平衡树要解决的问题。
      </>,
    );
    setBusy(false);
  ...
  const doReset = () => {
    ...
    setInput("");
    setRoot(buildSample());
    setMsg("回到示例树(完美平衡):7 个节点,高度只有 3。");
  };

  return (
    <div className="viz">
      <div className="viz-title">BST 实验室 —— 插入 / 查找 / 亲手养出一棵歪树</div>
      ...
      <div className="viz-ctl">
        ...
        <span className="mono dim" style={{ marginLeft: "auto", fontSize: 12 }}>
          节点 {count} · 高度 {count === 0 ? 0 : depth + 1} · 平衡下限 ⌈log₂(n+1)⌉ ={" "}
...
export function InorderFig() {
  ...
  return (
    <div className="viz">
      <div className="viz-title">为什么「中序遍历 = 升序」:把树垂直投影到地面</div>
      <div className="viz-stage">
        <svg viewBox="0 0 610 330" className="bst-svg" role="img" aria-label="中序遍历投影图">
          {IN_EDGES.map(([a, b], i) => (
            <line
              key={i}
              className="bst-edge"
      ...
      <div className="viz-msg">
        「左 &lt; 根 &lt; 右」对整棵子树成立 ⇒ 左子树<b>全体</b>画在根左边、右子树
        <b>全体</b>画在根右边 ⇒ 节点的水平位置天然有序。中序遍历(左 → 根 → 右)
        恰好就是从左到右逐个报数 —— 所以输出必然升序,不是巧合,是定义的直接推论。
      </div>
    </div>
  );
}
